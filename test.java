...
public class Test{
	public static void main(String[] args){
		...
		System.arraycopy(num,srcPos:1,num,destPos:2,length:3);
		//参数列表：源数组名,从哪个位置开始移动,移动到目标数组,
		//移动到目标数组的位置,移动原数组的长度
		
		线性表：由n个相同类型元素组成的一行线性结构
		：数组  线性表   
}
}
#### 线性表：
public class MyArry{
	...
	public void add(int data){
		if(size==num.length)
		{
			num=Arrays.copyOf(num,newlength:num.length<<1);
		}
		num[szie]=data;
		size++;
	...
	public void add(int pos,int data){
		...
		if(pos>size&&pos<num.length)
		{
			...
			System.arraycopy(num,pos,num,pos+1,1);
			num[pos]=data;
			num.size++;
	}
	...
	//
	public boolean set(int dest, int data){
