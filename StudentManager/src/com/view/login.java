...
public class login extends JFrame {
	...
	public login() {
		...
		label.setBounds(103, 13, 224, 40);
		contentPane.add(label);
		
		JLabel label_1 = new JLabel("用户名：");
		label_1.setFont(new Font("华文楷体", Font.PLAIN, 15));
		label_1.setBounds(68, 83, 72, 18);
		contentPane.add(label_1);
		
		JLabel label_2 = new JLabel("密  码：");
		label_2.setFont(new Font("华文楷体", Font.PLAIN, 15));
		label_2.setBounds(66, 134, 72, 18);
		contentPane.add(label_2);
		
		User_name = new JTextField();
		User_name.setBounds(149, 83, 190, 24);
		contentPane.add(User_name);
		User_name.setColumns(10);
		
		User_pwd = new JPasswordField();
		User_pwd.setBounds(151, 128, 189, 24);
		contentPane.add(User_pwd);
		
		JButton Login =  new JButton("登录");
		Login.setFont(new Font("华文楷体", Font.PLAIN, 15));
		Login.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					String sql="select * from [StudentList] where StudentId='"+userName+"' and StudentPassword='"+userPwd+"'";
		...
		Login.setBounds(149, 186, 72, 27);
		contentPane.add(Login);
		this.dispose();
		JButton reset = new JButton("取消");
		reset.setFont(new Font("华文楷体", Font.PLAIN, 15));
